...
// ler uma serie de ids dos jogadores e imprimi-los na saida em ordenados pela altura, utilizando o counting sort
...
public class CountingSort {
    ...
    public static void criarLog(long tempo) throws IOException {
        File logs = new File("803531_countingsort.txt");
    ...
    public static void ordemAlf(Jogador[] array, int tam) {
        for (int i = 0; i < tam; i++) {
            int menor = i;
    ...
    // COUTING SORT SORT pelo ano de nascimento do jogador
    public static void sort(Jogador[] array, int tam) {
        // ontar o numero de ocorrencias de cada elemento
        int[] cont = new int[getMaior(array, tam) + 1];
        Jogador[] ord = new Jogador[tam];

        // contagem das alturas
        for (int i = 0; i < tam; i++) {
            cont[array[i].getAltura()]++;
        }

        // atualiza o array de contagem para representar a posição correta de cada
        // elemento
        for (int i = 1; i < cont.length; i++) {
            cont[i] += cont[i - 1];
        }

        // constroi o array ordenado
        for (int i = tam - 1; i >= 0; i--) {
            int altura = array[i].getAltura();
            ord[cont[altura] - 1] = array[i];
            cont[altura]--;
        }

        System.arraycopy(ord, 0, array, 0, tam);
    }
