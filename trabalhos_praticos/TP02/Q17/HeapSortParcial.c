...
// heap sort parcial em C, ordenando jogadores pela altura
...
void criaLog()
{
    const char *nomeArquivo = "803531_heapparcial.txt";
    char content[100];
...
// encontra o elemento de maior id
Jogador getMax()
{
    Jogador maior = arrSort[0];
    for (int i = 1; i < tam; i++)
    {
        if (getId(maior) < getId(arrSort[i]))
        {
            maior = arrSort[i];
        }
    }
...
int getMaiorFilho(int i, int tam)
{
    ...
    if (esquerda < tam &&
        (getAltura(arrSort[esquerda]) > getAltura(arrSort[filho]) ||
         (getAltura(arrSort[esquerda]) == getAltura(arrSort[filho]) &&
          strcmp(getNome(arrSort[esquerda]), getNome(arrSort[filho])) > 0)))
    {
        filho = esquerda;
    }

    if (direita < tam &&
        (getAltura(arrSort[direita]) > getAltura(arrSort[filho]) ||
         (getAltura(arrSort[direita]) == getAltura(arrSort[filho]) &&
          strcmp(getNome(arrSort[direita]), getNome(arrSort[filho])) > 0)))
    {
        filho = direita;
    }
