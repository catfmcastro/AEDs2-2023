...
// radix sort em C, ordenando jogadores pelo id
...
void criaLog()
{
    const char *nomeArquivo = "803531_radixsort.txt";
    char content[100];
...
void radcountingSort(int exp)
{
    ...
    {
        output[count[(getId(arrSort[i]) / exp) % 10] - 1] = arrSort[i];
        count[(getId(arrSort[i]) / exp) % 10]--;
    }
...
{
    Jogador max = maxId();
    for (int exp = 1; getId(max) / exp > 0; exp *= 10)
    {
        radcountingSort(exp);
    }
}
